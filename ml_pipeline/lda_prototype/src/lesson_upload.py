...
user uploads a file in the flask logic
these functions are called to process the lesson, generate topic weights,
the weights and a unique lesson id are returned to the flask app to store in a db

TASKS:

[X]    - Lesson is processed (stemming, stopwords removed, count/vectorize)
[X]    - The pickled LDA model is loaded
[X]    - the get_topic_weights method is called on the document
[X]    - The output is formated in a 15 length np.array (for the 15 topics)
[ ]    - a unique lesson iD(hash of the text), and the lesson topic_array are returned to the flask app
